...
public:
  explicit EmptyFunctionMatcher(std::vector<StringRef> &emptyFunctions)
  ...
  void run(const MatchFinder::MatchResult &Result) override {
    ...
    if (FD && Manager.isWrittenInMainFile(
                  Manager.getSpellingLoc(FD->getLocStart()))) {
      emptyFunctions.push_back(FD->getName());
    }
  }
};

class EmptyFunctionsTransformer
    : public RecursiveASTVisitor<EmptyFunctionsTransformer> {
private:
  ASTContext &Context;
  SourceManager &sm;
...
public:
  explicit EmptyFunctionsTransformer(ASTContext &Context, Rewriter &OurRewriter,
      ...
      : Context(Context), sm(Context.getSourceManager()),
        OurRewriter(OurRewriter), emptyFunctions(emptyFunctions){};

  bool VisitFunctionDecl(const FunctionDecl *FD) {
    if (sm.isWrittenInMainFile(FD->getLocStart())) {
      if (std::find(emptyFunctions.begin(), emptyFunctions.end(),
                    FD->getName()) != emptyFunctions.end()) {
        auto text =
            getSourceTextStable(Context, sm.getSpellingLoc(FD->getLocStart()),
  ...
  bool VisitCallExpr(const CallExpr *Call) {
    if (sm.isWrittenInMainFile(Call->getLocStart()) &&
        Call->getDirectCallee()) {
      const auto &name = Call->getDirectCallee()->getName();
      if (std::find(emptyFunctions.begin(), emptyFunctions.end(), name) !=
          emptyFunctions.end()) {
        auto callText =
            getSourceTextStable(Context, sm.getSpellingLoc(Call->getLocStart()),
        ...
        if (deleteLine)
          OurRewriter.RemoveText(Call->getLocStart(), callText.size() + 1,
                                 options);
        else
...
public:
  explicit MyASTConsumer(ASTContext &Context, Rewriter &OurRewriter)
      : matcherCallback(emptyFunctions),
        transformer(Context, OurRewriter, emptyFunctions) {}

  void HandleTranslationUnit(ASTContext &Context) override {
    DeclarationMatcher functionMatcher =
        functionDecl(hasBody(compoundStmt(has(
                         compoundStmt(unless(hasAnySubstatement(stmt())))))))
            .bind("function");
