/**
 * This file implements the features that are not extracted correctly in the
 * Java implementation. We focus on the lexical features, in particular:
 *  - numFunctions
 *  - branchingFactor
 *  - nestingDepth
 * Moreover, we extract the following features where we feel more comfortable to
 * do that with the AST:
 *  - avgParams
 *  - stdDevNumParams
 *
 *
...
public:
  ...
  bool VisitStmt(Stmt *s) {
    ...
    if (sm.isWrittenInMainFile(srcloc)) {
      ...
      if (children.begin() == children.end()) {
        ...
        //                "\n";
        currentnestingdepth =
            findnestingdepth(currentnestingdepth, Context->getParents(*s));
      }
  ...
  bool VisitFunctionDecl(FunctionDecl *f) {
    ...
    bool inmainfile =
        Context->getSourceManager().isInMainFile(f->getLocation());
    if (inmainfile) {
  ...
  void extractlexicalfeatures() {
    ...
    this->avgparams = 1.0 *
                      std::accumulate(this->numberParams.begin(),
                                      this->numberParams.end(), 0UL) /
    ...
    double avgparamsbadhack =
        this->avgparams; // since following command has so namespace problems.
    std::transform(
        this->numberParams.begin(), this->numberParams.end(), diff.begin(),
        [avgparamsbadhack](double x) { return x - avgparamsbadhack; });
    double sq_sum =
    ...
    // E. branchingFactor
    this->avgbranchingfactor =
        1.0 *
        std::accumulate(this->branchingfactor.begin(),
...
private:
  ...
  bool checkifcontrolstmtorloop(const Decl *d) {
    return (isa<FunctionDecl>(d) || isa<NamespaceDecl>(d) ||
            isa<CXXRecordDecl>(d) || isa<CXXMethodDecl>(d));
  }

  /**
   * Get branching factor of current statement. It will be saved in class
   * variable branchingfactor.
   * @param s the current Stmt
   */
  void getbranchingfactor(Stmt *s) {
    const auto &children = s->children();

    // Case 1. Current stmt is a compound stmt, then count all control and
    // compound stmt's under this stmt. If nothing found, there are no { ctrl
    // stmt } under this stmt, so count 0
    ...
    // Case 2. Current stmt is single stmt without {...}. Check that no compound
    // stmt is under this stmt, if so, it will get a branching factor of 1.
    // Otherwise, we will visit the compound stmt later and work with it under
    // Case 1.
      ...
      for (auto par : parents) {
        ...
        if (ST) {
          if (checkifcontrolstmt(ST) || checkifcompoundcompoundpair(ST)) {
            ...
            //                    << ";" << nestingdepth
            //                           << "\t" << checkifcontrolstmt(ST) <<
            //                           ";"<<  checkifcompoundcompoundpair(ST)
            //                           << "\n";
          }
          // go to parent..
          temporarydepth =
              findnestingdepth(nestingdepth, Context->getParents(*ST));
        }
        ...
        if (DC) {
          if (checkifcontrolstmtorloop(DC)) {
            // increment depth if control statement
            nestingdepth++;
            //                    llvm::outs() << ">" << DC->getDeclKindName()
            //                    << ";" << nestingdepth << "\n";
          }
          temporarydepth =
              findnestingdepth(nestingdepth, Context->getParents(*DC));
        }
        // B.iii: Check if its a type, typeloc, nestednamespecifier,
        // nestnamespecifierloc
        //            const auto *TL = parents[0].get<Type>();
        //            const auto *TLloc = parents[0].get<TypeLoc>();
        //            const auto *NN = parents[0].get<NestedNameSpecifier>();
        //            const auto *NNloc =
        //            parents[0].get<NestedNameSpecifierLoc>(); if (TL || TLloc
        //            || NN || NNloc) {
