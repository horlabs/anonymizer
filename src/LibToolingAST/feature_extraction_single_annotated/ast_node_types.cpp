...
#include "../Utilities/Utils_Features_CodeLocs.h"
...
public:
  ...
  explicit GetNodeTypes(ASTContext *Context, std::string InFile,
                        bool get_avg_depth)
  ...
  bool VisitStmt(Stmt *s) {
    ...
    if (sm.isWrittenInMainFile(srcloc)) {
      ...
      auto featcoderange = getFeatureCodeRange(s->getLocStart(), s->getLocEnd(),
                                               sm, Context->getLangOpts());
      insertinhashmap_codelocations(s->getStmtClassName(), featcoderange,
                                    map_codelocations);
      ...
      if (this->get_avg_depth) {
        ...
        insertindepthhashmap(map, map_avgdepth, s->getStmtClassName(),
                             currentavgdepth);
        // Save the maximum depth by the way
        this->max_depth = (currentavgdepth > this->max_depth) ? currentavgdepth
                                                              : this->max_depth;
      }
    }
    return true;
  ...
  bool VisitDecl(Decl *d) {
    ...
    if (inmainfile) {
      ...
      auto featcoderange = getFeatureCodeRange(d->getLocStart(), d->getLocEnd(),
                                               sm, Context->getLangOpts());
      insertinhashmap_codelocations(d->getDeclKindName(), featcoderange,
                                    map_codelocations);
      ...
      if (this->get_avg_depth) {
        ...
        currentavgdepth = getdepthfornode(parents, this->Context, 0, InFile);
        insertindepthhashmap(map, map_avgdepth, d->getDeclKindName(),
                             currentavgdepth);
        // Save the maximum depth by the way
        this->max_depth = (currentavgdepth > this->max_depth) ? currentavgdepth
                                                              : this->max_depth;
...
//             : Visitor(Context, InFile, static_cast<bool>(cmdoption)),
//             cmdoption(cmdoption) {}
//
...
//        switch(cmdoption){
//            case onlytypes: printhashmap<int>(Visitor.map, Visitor.InFile);
//            break; case avg_depth: printhashmap<float>(Visitor.map_avgdepth,
//            Visitor.InFile); break; case max_depth: Visitor.printmaxdepth();
//            break;
//        }
//    }
// protected:
