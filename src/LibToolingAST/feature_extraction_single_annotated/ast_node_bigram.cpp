...
public:
  ...
  bool VisitStmt(Stmt *s) {
    // Check origin of stmt and if in main file: get its kind and the parent's
    // kind
    ...
    if (sm.isWrittenInMainFile(srcloc)) {
      ...
      auto featcoderange_child = getFeatureCodeRange(
          s->getLocStart(), s->getLocEnd(), sm, Context->getLangOpts());
      return createbigramwithparent(s->getStmtClassName(), featcoderange_child,
                                    parents);
    }
    return true;
  }
  ...
  bool VisitDecl(Decl *d) {
    ...
    if (inmainfile) {
      ...
      // Include this node together with its parent into hashmap as bigram
      auto featcoderange_child = getFeatureCodeRange(
          d->getLocStart(), d->getLocEnd(), sm, Context->getLangOpts());
      return createbigramwithparent(d->getDeclKindName(), featcoderange_child,
                                    parents);
    }
    return true;
  }
  ...
  bool VisitTypeLoc(TypeLoc x) {
    ...
    if (sm.isWrittenInMainFile(srcloc)) {
      ...
      // Include this node together with its parent into hashmap as bigram
      auto featcoderange_child = getFeatureCodeRange(
          x.getLocStart(), x.getLocEnd(), sm, Context->getLangOpts());
      return createbigramwithparent("Type", featcoderange_child, parents);
    }
    return true;
  }
...
private:
   ...
   * Otherwise, we check the type of the parent node and save the bigram in the
   * hashmap, and everyone is happy.
   * @param child - the child node's node type
   * @param featcoderange_child - the child node's source code range (row,
   * column of start and end)
  ...
  bool createbigramwithparent(const std::string &child,
                              FeatureCodeRange featcoderange_child,
    ...
    if (parents.empty()) {
      ...
      for (auto par : parents) {
        ...
        std::string bigram;
        std::shared_ptr<FeatureCodeRange> featcoderange_parent;
        if (ST) {
          bigram = createbigram(ST->getStmtClassName(), child);
          featcoderange_parent = std::make_shared<FeatureCodeRange>(
              getFeatureCodeRange(ST->getLocStart(), ST->getLocEnd(), sm,
                                  Context->getLangOpts()));
        }
        // B.ii: Check if its a declaration
          ...
          featcoderange_parent = std::make_shared<FeatureCodeRange>(
              getFeatureCodeRange(DC->getLocStart(), DC->getLocEnd(), sm,
                                  Context->getLangOpts()));
        }
        // B.iii: Check if its a type
          ...
          if (TLloc)
            featcoderange_parent = std::make_shared<FeatureCodeRange>(
                getFeatureCodeRange(TLloc->getLocStart(), TLloc->getLocEnd(),
                                    sm, Context->getLangOpts()));
        }
        // B.iv: Check if its a nested name ..
        else if (NN || NNloc) {
          //                    bigram = createbigram("NestedName", child);
          //                    llvm::errs() << "NestedName occured...look
          //                    deeper into it,"<< child << " file:" << InFile
          //                    << "\n";
        } else {
          llvm::errs() << "Bigram unknown parent class, " << child
                       << " file:" << InFile << "\n";
          return false;
        }
        if (!bigram.empty()) {
          insertinhashmap(bigram);
          if (featcoderange_parent) {
            auto featcoderange_combi = FeatureCodeRangeBigram(
                *featcoderange_parent, featcoderange_child);
            insertinhashmap_codelocations(bigram, featcoderange_combi,
                                          map_codelocations);
          }
        }
      }
