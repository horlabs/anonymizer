 ...
 * In VisitStmt, we really extract the respective source code that a leaf node
 * from the AST has. However, Joern also has more leaves than clang's AST, so we
 * also extract a. each named declaration (so that we get the name of a variable
 * or function) in VisitDecl b. each type, so that we get e.g. the type of a
 * variable declaration such as float f = 20.0; then we get "float".
 *
 * This file has two options that you can get via command-line argument
 * -ast-depth=true or =false. Either you just extract the occurence count of a
 * particular source code in the leaves. Or you extract the respective depth in
 * average.
 *
 * Caution: If you process a node, then call always insertinhashmap and then
 * insertindepthhashmap, because we calculate the average depth incrementally.
 * For this, we need to know how often a particular source code was already seen
 * - the job of insertinhashmap.
 *
 *
 * If ast-depth is false:
 * The output of this class is a json array with the format [ [a,b,c], [a,b,c],
 * ... ] where a is the 'code in ast leaf' - a sort of key. where b is the
 * #occurences. where c is a string that specifies the context of the 'code in
 * ast leaf'. For instance, if it's a VarDecl or a DeclRefExpr or a type. We
 * append each usage by '__<usage>' to the string, so that we get e.g. for
 * 'aVariable' the node ['aVariable', 4, 'Var__DeclRefExpr']. The 'aVariable'
 * was first declared as variable (-> Var) and later used (-> DeclRefExpr).
 *
 * If ast-depth is true:
 * The output of this class is a json array with the format [ [a,b], [a,b], ...
 * ] where a is the 'code in ast leaf' - a sort of key. where b is the average
 * depth.
 */
...
public:
  ...
  // a hashmap storing node and its code-location information
  std::unordered_map<std::string, std::vector<FeatureCodeRange>>
      map_codelocations;
  // a hashmap storing node and its usage context, e.g. is 'code in AST leaf' a
  // VarDecl, DeclRefExpr, Function?
  ...
  explicit CodeInASTLeaves(ASTContext *Context, std::string &InFile,
                           bool get_avg_depth)
      : InFile(InFile), Context(Context), sm(Context->getSourceManager()),
        get_avg_depth(get_avg_depth) {}
  ...
  bool VisitStmt(Stmt *s) {
    ...
    if (sm.isWrittenInMainFile(srcloc)) {
      ...
      if (children.begin() == children.end()) {
        ...
        auto featcoderange = getFeatureCodeRange(
            s->getLocStart(), s->getLocEnd(), sm, Context->getLangOpts());
        insertinhashmap_codelocations(escapesymbols(stmt2str(s)), featcoderange,
                                      map_codelocations);
        ...
        if (this->get_avg_depth) {
          ...
          insertindepthhashmap(map, map_avgdepth, escapesymbols(stmt2str(s)),
                               currentavgdepth);
        }
  ...
  bool VisitDecl(Decl *d) {
    ...
    if (inmainfile) {
      if (isa<NamedDecl>(d)) {
        ...
        auto featcoderange =
            getFeatureCodeRange(namedd->getLocStart(), namedd->getLocEnd(), sm,
                                Context->getLangOpts());
        insertinhashmap_codelocations(
            escapesymbols(namedd->getDeclName().getAsString()), featcoderange,
            map_codelocations);
        ...
        if (this->get_avg_depth) {
          ...
          currentavgdepth = getdepthfornode(parents, this->Context, 0, InFile);
          insertindepthhashmap(
              map, map_avgdepth,
              escapesymbols(namedd->getDeclName().getAsString()),
              currentavgdepth);
        }
...
#ifdef DEBUG
        llvm::outs() << "Decl:\t" << namedd->getDeclName().getAsString() << "("
                     << namedd->getDeclKindName() << ")"
                     << "\t" << currentavgdepth << "\n";
#endif
      }
  ...
  bool VisitTypeLoc(TypeLoc x) {
    ...
    if (sm.isWrittenInMainFile(srcloc)) {
      ...
      auto featcoderange = getFeatureCodeRange(x.getLocStart(), x.getLocEnd(),
                                               sm, Context->getLangOpts());
      insertinhashmap_codelocations(escapesymbols(x.getType().getAsString()),
                                    featcoderange, map_codelocations);
      ...
      if (this->get_avg_depth) {
        ...
        insertindepthhashmap(map, map_avgdepth,
                             escapesymbols(x.getType().getAsString()),
                             currentavgdepth);
      }
...
private:
  ...
  std::string stmt2str(Stmt *s) {
    std::string text = Lexer::getSourceText(
        CharSourceRange::getTokenRange(s->getSourceRange()), sm, LangOptions(),
        0);
    if (text.size() > 0 &&
        (text.at(text.size() - 1) == ',')) // the text can be ""
      return Lexer::getSourceText(
          CharSourceRange::getCharRange(s->getSourceRange()), sm, LangOptions(),
          0);
    return text;
  }

  /**
   * Insert the Class / Usage Type of current 'code in ast leaf' into the
   * hashmap 'map_usage'.
   * @param key  - the code in ast leaf
   * @param usage - the string representing its usage, e.g. StmtClassName or
   * DeclKindName...
   */
  void insertUsageInHashmap(std::string key, std::string usage) {
    // A. Just small check. Ensure that map also contains this key.
    if (map.find("\"" + key + "\"") == map.end()) {
      llvm::errs() << "Error: Storing Usage -- map does not have the #elements "
                      "for current str"
                   << "\n";
    }
...
//     bool get_avg_depth)
//             : Visitor(Context, InFile, get_avg_depth),
//             get_avg_depth(get_avg_depth) {}
//
...
//         if(!get_avg_depth){
//             printhashmap<int>(Visitor.map, Visitor.map_usage,
//             Visitor.InFile);
//         } else {
