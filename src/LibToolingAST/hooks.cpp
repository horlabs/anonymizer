// Compile with: g++ -Wall -O2 -Wl,--no-as-needed -fpic -shared -ldl -o hooks.so
// hooks.cpp
...
/*
 * If we replace external calls with tigress, the program will use dlsym to get
 * the addresses. In that case, our hooks will not work in all cases, so we have
 * also to hook dlsym to intercept these requests. But we have to ignore
 * requests from within our hooks
 */
...
void *dlsym(void *handle, const char *symbol) {
  ...
  if (real_dlsym == nullptr) {
    real_dlsym =
        reinterpret_cast<dlsym_t>(dlvsym(RTLD_NEXT, "dlsym", "GLIBC_2.2.5"));
    char *error = dlerror();
    if (error != nullptr) {
      fprintf(stderr, "%s\n", error);
      exit(1);
...
int system(const char *command) {
  ...
  if (real_system == nullptr) {
    real_system =
        reinterpret_cast<system_t>(__internal_dlsym(RTLD_NEXT, "system"));
    char *error = dlerror();
    if (error != nullptr) {
      fprintf(stderr, "%s\n", error);
      exit(1);
...
FILE *freopen(const char *filename, const char *mode, FILE *stream) {
  ...
  if (real_freopen == nullptr) {
    real_freopen =
        reinterpret_cast<reopen_t>(__internal_dlsym(RTLD_NEXT, "freopen"));
    char *error = dlerror();
    if (error != nullptr) {
      fprintf(stderr, "%s\n", error);
      exit(1);
  ...
  if (std::string(mode).find("r") != std::string::npos) {
    return stdin;
  } else if (std::string(mode).find("w") != std::string::npos ||
             std::string(mode).find("a") != std::string::npos) {
    return stdout;
  }
#ifdef DEBUG
...
FILE *fopen(const char *filename, const char *mode) {
  ...
  if (real_fopen == nullptr) {
    real_fopen =
        reinterpret_cast<fopen_t>(__internal_dlsym(RTLD_NEXT, "fopen"));
    char *error = dlerror();
    if (error != nullptr) {
      fprintf(stderr, "%s\n", error);
      exit(1);
...
void _ZNSt14basic_ifstreamIcSt11char_traitsIcEEC1EPKcSt13_Ios_Openmode(
    std::basic_ifstream<char, std::char_traits<char>> *self, const char *__s,
    std::ios_base::openmode __mode) {
  using ifstream_t =
      void (*)(std::basic_ifstream<char, std::char_traits<char>> *,
               const char *, std::ios_base::openmode);
  static ifstream_t real_ifstream = nullptr;
  if (real_ifstream == nullptr) {
    real_ifstream = reinterpret_cast<ifstream_t>(__internal_dlsym(
        RTLD_NEXT,
...
void _ZNSt14basic_ofstreamIcSt11char_traitsIcEEC1EPKcSt13_Ios_Openmode(
    std::basic_ofstream<char, std::char_traits<char>> *self, const char *__s,
    std::ios_base::openmode __mode) {
  using ofstream_t =
      void (*)(std::basic_ofstream<char, std::char_traits<char>> *,
               const char *, std::ios_base::openmode);
  static ofstream_t real_ofstream = nullptr;
  if (real_ofstream == nullptr) {
    real_ofstream = reinterpret_cast<ofstream_t>(__internal_dlsym(
        RTLD_NEXT,
...
void _ZNSt14basic_ofstreamIcSt11char_traitsIcEE4openEPKcSt13_Ios_Openmode(
    std::basic_ofstream<char, std::char_traits<char>> *self, const char *__s,
    std::ios_base::openmode __mode) {
  using ofstream_open_t =
      void (*)(std::basic_ofstream<char, std::char_traits<char>> *,
               const char *, std::ios_base::openmode);
  static ofstream_open_t real_ofstream = nullptr;
  if (real_ofstream == nullptr) {
    real_ofstream = reinterpret_cast<ofstream_open_t>(
        __internal_dlsym(RTLD_NEXT, "_ZNSt14basic_ofstreamIcSt11char_"
                                    "traitsIcEE4openEPKcSt13_Ios_Openmode"));
    char *error = dlerror();
    if (error != nullptr) {
      fprintf(stderr, "%s\n", error);
      exit(1);
...
void _ZNSt14basic_ifstreamIcSt11char_traitsIcEE4openEPKcSt13_Ios_Openmode(
    std::basic_ifstream<char, std::char_traits<char>> *self, const char *__s,
    std::ios_base::openmode __mode) {
  using ifstream_open_t =
      void (*)(std::basic_ifstream<char, std::char_traits<char>> *,
               const char *, std::ios_base::openmode);
  static ifstream_open_t real_ifstream = nullptr;
  if (real_ifstream == nullptr) {
    real_ifstream = reinterpret_cast<ifstream_open_t>(
        __internal_dlsym(RTLD_NEXT, "_ZNSt14basic_ifstreamIcSt11char_"
                                    "traitsIcEE4openEPKcSt13_Ios_Openmode"));
    char *error = dlerror();
    if (error != nullptr) {
      fprintf(stderr, "%s\n", error);
