...
int Ans() {
    ...
    if (TryY < 1000)
        if (TryMin != MinY[TryY]) return 0;
    if (TryX < 1000)
        if (TryMin != MinX[TryX]) return 0;
    ...
    if (TryY < 1000) {
        ...
        for (x = 0; x < Columns; x++) {
            MinX[x] = 1000;
            for (y = 0; y < Rows; y++)
                if (Map[y][x] < MinX[x]) MinX[x] = Map[y][x];
        }
        return 1;
    }
    if (TryX < 1000) {
        ...
        for (y = 0; y < Rows; y++) {
            MinY[y] = 1000;
            for (x = 0; x < Columns; x++)
                if (Map[y][x] < MinY[y]) MinY[y] = Map[y][x];
        }
        return 1;
    }
}
...
int main() {
    ...
    for (TCCount = 1; TCCount <= TestCases; TCCount++) {
        ...
        for (y = 0; y < Rows; y++)
            for (x = 0; x < Columns; x++) scanf("%d", &Map[y][x]);
